...
Every non-trivial decision made in producing this report is recorded below. A decision is non-trivial if a competent engineer could reasonably have chosen differently. This table is the single source of truth for all "why" rationale; no rationale is embedded in inline comments within the report body.

| # | Decision | Alternatives Considered | Rationale | Risks |
| - | -------- | ----------------------- | --------- | ----- |
| D1 | Place report at `docs/AGC_ENGINEERING_ANALYSIS.md` in a new `docs/` directory | (a) Root-level `AGC_ENGINEERING_ANALYSIS.md`; (b) `Comanche055/ANALYSIS.md` or `Luminary099/ANALYSIS.md` inside source directories | Root-level placement clutters the repository alongside `README.md`, `CONTRIBUTING.md`, and `LICENSE.md`, which serve repository management rather than analysis. A dedicated `docs/` directory signals that the file is supplementary analysis, not project infrastructure. Placing inside a source directory would incorrectly scope the report to one module when it covers both. | The `docs/` directory is new and not referenced by the root `README.md` (which we cannot modify per read-only constraint). Discovery requires navigating to the directory manually. |
| D2 | Use Comanche055 (CM) as the primary reference with LM differences noted as deltas | (a) Treat Luminary099 (LM) as primary; (b) Present both equally in parallel throughout; (c) Alternate primary by section | CM and LM share ~90% of scheduling, alarm, and restart architecture. Comanche055 was assembled earlier (Apr 1, 1969 vs Jul 14, 1969) and represents the baseline from which LM diverged. Dual-primary would double section length without proportional insight. LM-specific deltas (8th core set, BAILOUT1/POODOO1, GOPOODOO) are concentrated and better served by comparison tables. | LM-unique patterns (SERVICER state handling in GOPOODOO, FILLED routine in waitlist) receive less narrative depth than CM equivalents. Readers primarily interested in lunar descent software must cross-reference two subsections. |
| D3 | Section ordering: Memory, Scheduling, Error Handling, Naming, Modern Context | (a) Chronological by mission phase; (b) Alphabetical; (c) Error handling first (most famous topic); (d) Bottom-up from hardware to software | Memory constraints are the foundational "why" behind all other architecture. Understanding bank-switching is prerequisite to understanding why the Executive fits in ~500 lines. Scheduling must precede error handling because alarms reference Executive/Waitlist overflow. Naming conventions span all prior sections. Modern context synthesizes everything. This ordering minimizes forward references. | A reader interested specifically in the 1202 alarms must read through two full sections before reaching Section 3. |
| D4 | Include five Mermaid diagrams inline rather than external image files | (a) ASCII art diagrams; (b) External PNG/SVG images; (c) No diagrams, prose-only; (d) PlantUML syntax | GitHub natively renders Mermaid since November 2022, requiring no build step or external assets. Mermaid is version-control-friendly (text diffs), self-contained in the markdown, and does not require any tooling installation. ASCII art is fragile under reformatting. External images require hosting and break the single-file deliverable constraint. | Mermaid rendering depends on the viewing platform. GitLab, Bitbucket, and local markdown editors may not render Mermaid without plugins. Diagram complexity is limited by Mermaid's flowchart grammar. |
| D5 | Curate a subset of ~25 alarm codes for the appendix rather than reproducing the full catalog (~60 codes) | (a) Full catalog reproduction; (b) No alarm table, prose references only; (c) Only the "famous" alarms (1201, 1202, 1203) | The full catalog includes many subsystem-specific alarms (radar, optics, DAP) that lack architectural significance outside their module. A curated subset covering all severity levels, all referenced alarm tiers, and the mission-critical codes provides sufficient reference without bulk. Including only famous alarms would omit the severity classification system that the table is meant to illustrate. | Alarm codes omitted from the subset may be relevant to readers investigating specific subsystems. The report explicitly cites the source file and line range for the full catalog, enabling direct lookup. |
| D6 | Use direct source line number citations (e.g., "line 145") alongside label names | (a) Label-only citations without line numbers; (b) Line ranges without labels; (c) Page-number citations matching MIT printout pagination | Line numbers enable precise programmatic lookup (`sed -n '145p' Comanche055/EXECUTIVE.agc`). Labels alone require grep. Page numbers from the original MIT printouts (visible as `# Page NNNN` comments) would be historically accurate but are not how developers interact with the GitHub-hosted files. Combining labels and line numbers maximizes accessibility. | Line numbers are fragile — any future edit to the repository would invalidate them. However, this repository is a historical preservation project where files are corrected to match scans, not actively developed, making line drift unlikely. |
| D7 | Describe FINDVAC's VAC scan as "sequential check of five areas" rather than reproducing the CCS chain | (a) Reproduce the full CCS VAC1USE/VAC2USE/VAC3USE/VAC4USE/VAC5USE instruction sequence; (b) Pseudocode representation; (c) Architectural summary only | The user directive specifies "explain WHY, not WHAT line-by-line." The CCS chain is a WHAT detail; the architectural insight is that five fixed VAC areas create a hard resource ceiling. Reproducing assembly would shift the report toward code walkthrough. | Readers wanting to understand the specific CCS fall-through pattern must consult the source directly. The report trades implementation completeness for architectural clarity. |
| D8 | Report file count as 175 total (85 CM + 90 LM) based on actual `find` count | (a) Use AAP-stated counts of 86 CM + 92 LM = 178; (b) Use MAIN.agc `$`-include counts (84 CM + 89 LM) | Filesystem `find *.agc` returns 85 files for Comanche055 and 90 for Luminary099. MAIN.agc includes 84 and 89 respectively (the MAIN.agc files themselves are the 85th and 90th). The AAP counts (86 and 92) appear to include non-existent files or count differently. The report uses verified filesystem counts. | If the AAP counts reflect a different branch or additional files not present in the current repository state, the report's counts would be inconsistent with the AAP narrative. |
| D9 | Describe the LM's `ABORT EQUALS WHIMPER` vs CM's `ABORT EQUALS BAILOUT` as a severity difference | (a) Treat as an implementation detail without interpretation; (b) Attribute to independent development teams rather than design intent | The `EQUALS` directive creates a deliberate symbolic alias. In the CM, calling `ABORT` invokes `BAILOUT` (restart with recovery). In the LM, calling `ABORT` invokes `WHIMPER` (restart entry, less aggressive). The LM's powered-descent context makes aggressive abort more dangerous (no abort tower, limited abort windows), providing a clear engineering rationale for the softer default. | The interpretation assumes the difference is intentional rather than an artifact of independent development schedules. The `# *** TEMPORARY` comment on the CM ABORT EQUALS line (line 230) suggests the CM mapping was provisional, complicating the "deliberate design" narrative. |
| D10 | Include T.S. Eliot attribution for WHIMPER label | (a) Omit literary attribution, cite only the technical behavior; (b) Include the full Eliot quote | The name "WHIMPER" in the context of a restart routine (the system ends "not with a bang but a whimper") is a deliberate literary reference consistent with the naming conventions section. Attribution adds cultural context without prose bloat. Reproducing the full quote would be unnecessary. | Attribution is inferential — no source comment explicitly cites Eliot. However, the reference is widely recognized in AGC literature and the naming pattern (CURTAINS, BAILOUT, POODOO) confirms the programmers' penchant for culturally loaded labels. |
| D11 | Use octal notation (OCT prefix) for all alarm codes | (a) Decimal equivalents; (b) Hexadecimal; (c) Both octal and decimal | The AGC source uses OCT notation exclusively for alarm codes. The `ASSEMBLY_AND_OPERATION_INFORMATION.agc` alarm catalog lists all codes in octal. Using decimal or hex would require readers to perform conversion when cross-referencing the source. Matching source convention eliminates this friction. | Readers unfamiliar with octal must mentally convert to understand magnitudes. Modern engineers are more accustomed to decimal and hexadecimal. |
| D12 | Modern Context section uses architectural analogies rather than direct code comparisons | (a) Include side-by-side code snippets (AGC vs modern language); (b) Cite specific modern frameworks by version; (c) Omit modern context entirely | The user specifically requested "light modern context" comparing AGC patterns with contemporary engineering. Direct code comparisons would be anachronistic (1960s assembly vs 2020s high-level languages) and misleading. Framework-specific citations (e.g., "Kubernetes v1.28 pod eviction") would date rapidly. Architectural pattern names (load shedding, WAL, bulkhead isolation) are stable terminology. | Architectural analogies may oversimplify both the AGC implementation and modern equivalents. Readers expert in either domain may find the comparisons superficial. |

### Deviation Register

Any deviation from a literal or obvious interpretation of the AAP requirements is recorded here.

| # | Requirement (AAP) | Deviation | Justification |
| - | ----------------- | --------- | ------------- |
| V1 | AAP states "86 .agc files" for Comanche055 and "92 .agc files" for Luminary099 | Report states 85 and 90 respectively | Verified via `find Comanche055 -name "*.agc" \| wc -l` returning 85 and `find Luminary099 -name "*.agc" \| wc -l` returning 90. Report uses filesystem-verified counts. |
| V2 | AAP Section 0.4.1 specifies "CCSHOLE and defensive programming patterns" under Error Handling | CCSHOLE documented in Section 3 (Error Handling) rather than receiving a standalone subsection | CCSHOLE is a single-pattern item (6 lines of code). A dedicated subsection would be disproportionate. It is documented as a named subsection within Section 3 with full file path, line number, alarm code, and engineering rationale. |
| V3 | AAP specifies "VARALARM" as part of the alarm system documentation | VARALARM referenced by label in the LM grep output but not given a dedicated paragraph | VARALARM (`Luminary099/ALARM_AND_ABORT.agc`, line 216) is a convenience entry point that merges with the main ALARM path. Its behavior is identical to ALARM except for calling convention. Documenting it separately would add words without architectural insight. The label appears in the source citation for the LM alarm file. |
| V4 | AAP specifies five Mermaid diagrams including "Waitlist Timer Architecture" as a separate diagram | Waitlist timer architecture is combined as a second diagram within Section 2 rather than a standalone section | The waitlist and Executive diagrams are co-located in Section 2 where both scheduling models are discussed. Separating them would require the reader to navigate away from the scheduling narrative. |
| V5 | AAP Section 0.5.2 specifies "30+ specific .agc files" as key citations | Report explicitly cites approximately 25 distinct .agc files by name | The 25 cited files cover all five decision domains comprehensively. Additional files would either duplicate evidence already presented or reference subsystem-specific modules (e.g., individual P-programs) outside the architectural analysis scope. |

## Appendix: Source-to-Report Traceability Matrix

Bidirectional mapping between AGC source constructs and their documentation in this report. Every source file, label, and routine cited in the report body is listed with its report section. Every report section is listed with its source evidence.

### Forward Traceability: Source Construct to Report Section

| Source File | Key Constructs | Report Section(s) | Coverage |
| ----------- | -------------- | ------------------ | -------- |
| `Comanche055/INTER-BANK_COMMUNICATION.agc` | BANKCALL (line 41), SWCALL (line 48), SWRETURN (line 56), POSTJUMP (line 64), BANKJUMP (line 70), SUPDACAL (line 84), SUPERSW (line 176) | Section 1 (Core-Rope Memory and Bank-Switching) | All 7 public entry points documented |
| `Comanche055/ERASABLE_ASSIGNMENTS.agc` | M/SIZE/N notation (lines 66-95), EQUALS directive (lines 44-59), NEWJOB LOC 67, hardwired registers (lines 101-107) | Section 1 (Erasable Register Allocation), Section 4 (M/SIZE/N Notation) | Notation system, sharing mechanism, hardwired addresses documented |
| `Luminary099/ERASABLE_ASSIGNMENTS.agc` | LM erasable allocation (2,635 lines vs CM 3,785) | Section 1 (CM vs LM Erasable Budget) | Line count comparison, mission-profile rationale documented |
| `Comanche055/EXECUTIVE.agc` | NOVAC (line 37), FINDVAC (line 52), CHANJOB (line 211), JOBSLEEP (line 90), JOBWAKE (line 97), ENDOFJOB (line 117), DUMMYJOB (line 455), NO.CORES DEC 6 (line 161), BAILOUT OCT 1201 (lines 145-146), BAILOUT OCT 1202 (lines 204-205) | Section 2 (Executive, 1202/1201 Alarms, CM vs LM Core Sets), Section 5 (Watchdog/Heartbeat, Load Shedding) | All major entry points, resource limits, context switch mechanism documented |
| `Luminary099/EXECUTIVE.agc` | NO.CORES DEC 7 (line 162), DUMMYJOB (line 464), BAILOUT1 usage (line 146) | Section 2 (CM vs LM Core Sets comparison table) | 8th core set rationale, LM-specific calling convention documented |
| `Comanche055/WAITLIST.agc` | LST1/LST2 arrays, T3RUPT dispatch, TWIDDLE, FIXDELAY, VARDELAY, LONGCALL, WTABORT OCT 1203 (lines 339-340) | Section 2 (Waitlist), Section 4 ("NO ROOM IN THE INN") | Timer architecture, overflow handling, task slot count documented |
| `Luminary099/WAITLIST.agc` | FILLED routine, WAITPOOH/LONGPOOH, POODOO1 OCT 1204 | Section 2 (Waitlist — LM differences) | LM overflow handling and alternate error paths documented |
| `Comanche055/ALARM_AND_ABORT.agc` | ALARM (line 53), BAILOUT (line 143), WHIMPER (line 156), POODOO (line 166), CURTAINS (line 205), CCSHOLE (line 197), DOALARM EQUALS ENDOFJOB (line 211), FAILREG cascade (lines 74-105), PROGLARM (line 92), MULTFAIL (line 101), VAC5STOR, ABORT EQUALS BAILOUT (line 230) | Section 3 (Three-Tier Hierarchy, FAILREG, CCSHOLE, DOALARM), Section 4 (Abort Naming Hierarchy) | All entry points, storage mechanism, defensive patterns, naming analysis documented |
| `Luminary099/ALARM_AND_ABORT.agc` | BAILOUT1 (line 189), POODOO1 (line 197), GOPOODOO (line 162), ABORT EQUALS WHIMPER (line 234), DOALARM (line 212) | Section 3 (LM-Specific Alarm Differences) | LM alternate entries, SERVICER state handling, default severity difference documented |
| `Comanche055/FRESH_START_AND_RESTART.agc` | SLAP1 (line 145), DOFSTART (line 169), GOPROG (line 290), ENEMA (line 393), MR.KLEAN (line 264), P00KLEAN (line 268), V37KLEAN (line 274), ERESTORE validation | Section 3 (Four Restart Pathways), Section 4 (ENEMA, MR.KLEAN naming) | All four pathways, cleanup cascade, ERESTORE check documented |
| `Comanche055/PHASE_TABLE_MAINTENANCE.agc` | PHASCHNG Type A/B/C (line 85+), 2PHSCHNG, NEWMODEX (line 39), TBASE/LONGBASE | Section 2 (Phase Table Restart Protection), Section 5 (Checkpointing vs WAL) | Three encoding types, dual-phase change, timing reference documented |
| `Comanche055/RESTARTS_ROUTINE.agc` | ITSAVAR (line 69), ITSATBL (line 178), FINDTIME (line 130) | Section 3 (Phase Group Restart Dispatch) | Variable vs table dispatch, delta-T recovery documented |
| `Comanche055/RESTART_TABLES.agc` | Job/waitlist/longcall entry encoding, priority sign convention | Section 3 (Phase Group Restart Dispatch — ITSATBL subsection) | Table format, sign convention for FINDVAC/NOVAC documented |
| `Comanche055/INTERRUPT_LEAD_INS.agc` | SETLOC 4000 (line 32), GOPROG vector (lines 36-39), T3RUPT vector (lines 51-54), T5RUPT vector (lines 46-49), T3RPTBB (line 95) | Section 2 (Waitlist — T3RUPT dispatch) | Interrupt vector table, T3RUPT/T5RUPT dispatch mechanism documented |
| `Luminary099/BURN_BABY_BURN--MASTER_IGNITION_ROUTINE.agc` | Magnificent Montague origin (lines 33-44), HONI SOIT QUI MAL Y PENSE (line 66), NOLI SE TANGERE (line 72) | Section 4 (Cultural References, Latin Phrases) | Naming origin, Latin warnings documented |
| `Luminary099/THE_LUNAR_LANDING.agc` | FLAGORGY (line 64), "DIONYSIAN FLAG WAVING" (line 64), "OFF TO SEE THE WIZARD" (line 254), "PLEASE CRANK THE SILLY THING AROUND" (lines 245-246) | Section 4 (Sardonic Humor) | All four cultural/humor references documented |
| `Comanche055/PINBALL_GAME_BUTTONS_AND_LIGHTS.agc` | PINBALL naming (line 37 header), verb-noun DSKY interface | Section 4 (Cultural References — PINBALL) | Naming origin and interface metaphor documented |
| `Luminary099/ASSEMBLY_AND_OPERATION_INFORMATION.agc` | Alarm code catalog (lines 900-1021), severity markers (lines 1014-1021) | Appendix: Key Alarm Codes Reference | 25 codes curated from full catalog, severity legend reproduced |
| `Comanche055/INTERPRETER.agc` / `Luminary099/INTERPRETER.agc` | Interpretive pseudo-instruction set | Section 1 (Interpretive Language as Memory Compression) | Memory compression rationale, speed tradeoff documented |
| `Comanche055/TAGS_FOR_RELATIVE_SETLOC.agc` / `Luminary099/TAGS_FOR_RELATIVE_SETLOC.agc` | SETLOC directives, bank layout mapping | Section 1 (Interpretive Language — bank layout reference) | Logical-to-physical bank mapping referenced |
| `Comanche055/FIXED_FIXED_CONSTANT_POOL.agc` / `Luminary099/FIXED_FIXED_CONSTANT_POOL.agc` | Shared constants in non-banked memory | Section 1 (Interpretive Language — constant sharing) | Constant pool optimization documented |
| `Comanche055/MAIN.agc` / `Luminary099/MAIN.agc` | Include manifests (84/89 modules) | Preamble (file counts, assembly dates) | Module counts, organizational structure documented |

### Reverse Traceability: Report Section to Source Evidence

| Report Section | Source Files Referenced | Constructs Cited | Minimum Citations Met |
| -------------- | ---------------------- | ---------------- | --------------------- |
| Preamble | `Comanche055/MAIN.agc`, `Luminary099/MAIN.agc` | File counts, assembly dates, module organization | Yes (2 files) |
| 1. Memory and Resource Constraints | `Comanche055/INTER-BANK_COMMUNICATION.agc`, `Comanche055/ERASABLE_ASSIGNMENTS.agc`, `Luminary099/ERASABLE_ASSIGNMENTS.agc`, `Comanche055/INTERPRETER.agc`, `Luminary099/INTERPRETER.agc`, `*/TAGS_FOR_RELATIVE_SETLOC.agc`, `*/FIXED_FIXED_CONSTANT_POOL.agc` | BANKCALL, SWCALL, SWRETURN, POSTJUMP, BANKJUMP, SUPERSW, SUPDACAL, M/SIZE/N notation, EQUALS, NEWJOB LOC 67 | Yes (7+ files, 10+ constructs) |
| 2. Real-Time Scheduling | `Comanche055/EXECUTIVE.agc`, `Luminary099/EXECUTIVE.agc`, `Comanche055/WAITLIST.agc`, `Luminary099/WAITLIST.agc`, `Comanche055/PHASE_TABLE_MAINTENANCE.agc`, `Comanche055/INTERRUPT_LEAD_INS.agc` | NOVAC, FINDVAC, CHANJOB, JOBSLEEP, JOBWAKE, ENDOFJOB, DUMMYJOB, NO.CORES, LST1/LST2, T3RUPT, TWIDDLE, LONGCALL, WTABORT, PHASCHNG, 2PHSCHNG, NEWMODEX | Yes (6+ files, 16+ constructs) |
| 3. Error Handling | `Comanche055/ALARM_AND_ABORT.agc`, `Luminary099/ALARM_AND_ABORT.agc`, `Comanche055/FRESH_START_AND_RESTART.agc`, `Comanche055/RESTARTS_ROUTINE.agc`, `Comanche055/RESTART_TABLES.agc` | ALARM, BAILOUT, POODOO, WHIMPER, CURTAINS, CCSHOLE, DOALARM, FAILREG, PROGLARM, MULTFAIL, SLAP1, DOFSTART, GOPROG, ENEMA, MR.KLEAN, ITSAVAR, ITSATBL, FINDTIME, BAILOUT1, POODOO1, GOPOODOO | Yes (5+ files, 21+ constructs) |
| 4. Naming Conventions | `Luminary099/BURN_BABY_BURN--MASTER_IGNITION_ROUTINE.agc`, `Luminary099/THE_LUNAR_LANDING.agc`, `Comanche055/PINBALL_GAME_BUTTONS_AND_LIGHTS.agc`, `Comanche055/ALARM_AND_ABORT.agc`, `Comanche055/ERASABLE_ASSIGNMENTS.agc` | BURN BABY BURN, PINBALL, FLAGORGY, HONI SOIT, NOLI SE TANGERE, POODOO, WHIMPER, CURTAINS, ENEMA, MR.KLEAN, M/SIZE/N notation, CHANJOB, NEWPRIO, ALMCADR, PHASCHNG | Yes (5+ files, 15+ constructs) |
| 5. Modern Context | All files from Sections 1-4 | BAILOUT/1202 (load shedding), DUMMYJOB (heartbeat), PHASCHNG/RESTARTS (checkpointing), ALARM/BAILOUT/POODOO (graceful degradation), VAC/core set exhaustion (resource pools) | Yes (cross-cutting, 5+ architectural patterns) |
| Appendix: Alarm Codes | `Luminary099/ASSEMBLY_AND_OPERATION_INFORMATION.agc` | 25 alarm codes with severity markers, source modules, descriptions | Yes (1 primary file, comprehensive catalog subset) |
